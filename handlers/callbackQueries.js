...
async function showManagerOrderDetails(bot, chatId, messageId, orderMongoId) {
  try {
    ...
    if (!order) {
      console.error(
        `[showManagerOrderDetails] Order not found for Mongo ID: ${orderMongoId}`
      );
      return bot.editMessageText("Помилка: Замовлення не знайдено.", {
        ...
        reply_markup: {
          inline_keyboard: [
            [
              {
                text: "◀️ Назад до списку замовлень",
    ...
    if (!order.orderId) {
      console.error(
        `[showManagerOrderDetails] Order ID (system ID) is missing for Mongo ID: ${orderMongoId}`
      );
    }
...
🚚 Доставка:
   ...
   - Вартість доставки: ${
     order.deliveryPrice != null ? order.deliveryPrice + " грн" : "Не вказано"
   }
...
📅 Створено: ${createdAtDate}
    `;
    console.log(
      `[showManagerOrderDetails] Displaying details for Order ID: ${order.orderId}`
    );
    ...
    if (order.orderId) {
      keyboard.unshift([
        {
          text: "📊 Змінити статус",
          callback_data: `change_status_${order.orderId}`,
        },
      ]);
    }
    ...
    console.error(
      `[showManagerOrderDetails] Error showing details for Mongo ID ${orderMongoId}:`,
      error
    );
    await bot
      .editMessageText("Не вдалося завантажити деталі замовлення.", {
        chat_id: chatId,
        message_id: messageId,
        reply_markup: {
          inline_keyboard: [
      ...
      .catch((editErr) =>
        console.error("Failed to edit message on error:", editErr)
      );
  }
}
...
async function callbackQueryHandler(bot, query, photoUploadState, orderData) {
  ...
  console.log(
    `[CALLBACK_QUERY] Data: ${data}, UserID: ${userId}, ChatID: ${chatId}, ManagerID Env: ${managerId}`
  );
  ...
  if (data && data.startsWith("view_order_")) {
    console.log(
      `[CALLBACK_QUERY] Condition data.startsWith("view_order_") is TRUE`
    );
    if (isManager) {
      console.log(
        `[CALLBACK_QUERY] Condition isManager is TRUE. Processing view_order_${
          data.split("_")[2]
        }...`
      );
      try {
        ...
        console.log(
          `[CALLBACK_QUERY] Extracted Order Mongo ID: ${orderMongoId}`
        );
        await showManagerOrderDetails(bot, chatId, messageId, orderMongoId);
        console.log(
          `[CALLBACK_QUERY] showManagerOrderDetails finished successfully.`
        );
      } catch (error) {
        console.error(
          "[CALLBACK_QUERY] Error inside 'view_order_' processing:",
          error
        );
        try {
          ...
          console.error("Failed to answer callback query on error:", ansErr);
        }
      }
      return;
    } else {
      console.log(
        `[CALLBACK_QUERY] Condition isManager is FALSE. Ignoring 'view_order_' for non-manager.`
      );
      bot.answerCallbackQuery(query.id, {
        text: "Ця дія доступна лише менеджеру.",
      });
      return;
    }
  }
  ...
  if (data === "back_to_order_list") {
    console.log(
      `[CALLBACK_QUERY] Condition data === "back_to_order_list" is TRUE`
    );
    if (isManager) {
      console.log(
        `[CALLBACK_QUERY] Condition isManager is TRUE. Processing back_to_order_list...`
      );
      try {
        bot.answerCallbackQuery(query.id);
        await showManagerOrdersList(bot, chatId, messageId);
        console.log(
          `[CALLBACK_QUERY] showManagerOrdersList (back) finished successfully.`
        );
      } catch (error) {
        console.error(
          "[CALLBACK_QUERY] Error inside 'back_to_order_list' processing:",
          error
        );
        try {
          await bot.answerCallbackQuery(query.id, {
            text: "Помилка повернення до списку",
            show_alert: true,
          });
        } catch (ansErr) {
          console.error("Failed to answer callback query on error:", ansErr);
      ...
      console.log(
        `[CALLBACK_QUERY] Condition isManager is FALSE. Ignoring 'back_to_order_list' for non-manager.`
      );
      bot.answerCallbackQuery(query.id);
      return;
    }
  ...
  if (data.startsWith("change_status_") && isManager) {
    ...
    console.log(
      `[CALLBACK_QUERY] Manager requesting status change for Order ID: ${orderId}`
    );
    return bot.editMessageText(
      `Виберіть новий статус для замовлення ID: ${orderId} (@${order.username})\nПоточний: ${currentStatus}`,
      {
        chat_id: chatId,
        message_id: messageId,
        reply_markup: {
  ...
  if (data.startsWith("set_status_") && isManager) {
    ...
    console.log(
      `[CALLBACK_QUERY] Attempting to set status for Order ID: ${orderId}, StatusIndex: ${statusIndex}, Action: ${action}`
    );
    ...
    if (!order) {
      console.error(
        `[CALLBACK_QUERY] Order not found for ID: ${orderId} during status update.`
      );
      return bot.answerCallbackQuery(query.id, {
        text: "Замовлення не знайдено.",
      });
    ...
    switch (statusIndex) {
      ...
      case "4":
        if (action === "photo") {
          ...
          if (!user) {
            console.error(
              `[CALLBACK_QUERY] Client user @${order.username} not found for photo report.`
            );
            return bot.answerCallbackQuery(query.id, {
          ...
          console.log(
            `[CALLBACK_QUERY] Initiating photo upload state for Order ID: ${orderId}, Client ID: ${user.user_id}`
          );
          return bot.editMessageText(
            "Будь ласка, надішліть фото. Коли завершите, натисніть кнопку 'Надіслати всі фото'.",
            {
              chat_id: chatId,
              message_id: messageId,
              reply_markup: {
                  ...
                  [
                    {
                      ...
                      callback_data: `cancel_photo_upload_${orderId}`,
                    },
                  ],
                ],
              },
      ...
      case "5":
        if (action === "confirm") {
          ...
          console.warn(
            `[CALLBACK_QUERY] Unknown action '${action}' for status index 5.`
          );
          notifyClient = false;
        }
        break;
      case "6":
        if (action === "pack") {
          ...
          console.log(
            `[CALLBACK_QUERY] Requesting delivery price for Order ID: ${orderId}`
          );
          notifyClient = false;
          return bot.editMessageText(
            `Введіть вартість доставки для замовлення ID: ${orderId} (@${order.username})`,
            {
              chat_id: chatId,
              message_id: messageId,
            }
          ...
          console.warn(
            `[CALLBACK_QUERY] Unknown action '${action}' for status index 6.`
          );
          notifyClient = false;
        }
        break;
      case "7":
        statusMessage =
          "Посилка прибула до України та готується до відправлення ✅";
        break;
      case "8":
        if (action === "sent") {
          ...
          console.warn(
            `[CALLBACK_QUERY] Unknown action '${action}' for status index 8.`
          );
          notifyClient = false;
        }
        break;
    ...
    if (statusMessage) {
      ...
      await order.save();
      console.log(
        `[CALLBACK_QUERY] Order ID: ${orderId} status updated to: ${statusMessage}`
      );
      ...
      if (notifyClient) {
        ...
        if (user) {
          try {
            ...
            console.log(
              `[CALLBACK_QUERY] Client @${order.username} notified about status update.`
            );
          } catch (clientNotifyError) {
            console.error(
              `[CALLBACK_QUERY] Failed to notify client @${order.username} (ID: ${user.user_id}):`,
              clientNotifyError.message
            );
          }
        } else {
          console.error(
            `[CALLBACK_QUERY] Client user @${order.username} not found for status notification.`
          );
        }
      }
    } else if (
      ...
      await bot.editMessageText(
        `Дія для статусу ${statusIndex} оброблена, але статус не змінено.`,
        {
          chat_id: chatId,
          message_id: messageId,
          reply_markup: {
  ...
  if (data.startsWith("quick_reply_")) {
    ...
    switch (data) {
      ...
      case "quick_reply_7":
        ...
        if (existingTicket) {
          await bot.sendMessage(chatId, "У вас вже є активна заявка.");
        } else {
          ...
          if (currentHour >= 21 || currentHour < 9) {
            await bot.sendMessage(
              chatId,
              `Менеджери відпочивають🥱 Зачекайте до 9:00.`
            );
            await bot.sendMessage(chatId, `✍️ Напишіть питання...`);
          } else {
            await bot.sendMessage(
              chatId,
              `Дякуємо, ${greetingName}, очікуйте менеджера 😉`
            );
          }
    ...
    if (!["quick_reply_7", "quick_reply_menu"].includes(data)) {
      await bot.answerCallbackQuery(query.id);
    }
    return;
  }
  ...
  if (data.startsWith("accept_") && isManager) {
    ...
    if (ticket) {
      if (ticket.accepted) {
        console.log(
          `[CALLBACK_QUERY] Ticket ${ticketId} already accepted. Setting as active conversation.`
        );
        ...
        await ticket.save();
        await bot.answerCallbackQuery(query.id, {
          text: `Переключено на заявку ${ticketId}`,
        });
        await bot.editMessageText(
          `Ви переключились на заявку ${ticketId}. Напишіть повідомлення клієнту.`,
          {
            chat_id: chatId,
            message_id: messageId,
            reply_markup: {
              inline_keyboard: [
                [
                  {
                    text: "Завершити листування",
                    callback_data: `close_${ticketId}_manager`,
                  },
                ],
              ],
            },
        ...
        await ticket.save();
        console.log(
          `[CALLBACK_QUERY] Ticket ${ticketId} accepted and set as active.`
        );
        ...
        if (ticket.messages.length > 0) {
          ...
          if (userMessages.length > 0) {
            ...
            userMessages.forEach((msg) => {
              ...
              const msgTime = `${msgDate.getHours()}:${msgDate
                .getMinutes()
                .toString()
                .padStart(2, "0")}`;
        ...
        await bot.answerCallbackQuery(query.id, {
          text: `Заявку ${ticketId} прийнято`,
        });
        await bot.editMessageText(
          `Заявка ${ticketId} прийнята. Напишіть повідомлення клієнту.`,
          {
            chat_id: chatId,
            message_id: messageId,
            reply_markup: {
      ...
      console.error(
        `[CALLBACK_QUERY] Ticket ${ticketId} not found for acceptance.`
      );
      await bot.answerCallbackQuery(query.id, {
      ...
      await bot
        .deleteMessage(chatId, messageId)
        .catch((e) => console.error("Failed to delete msg:", e.message));
    }
    return;
    ...
    if (!ticket) {
      console.error(
        `[CALLBACK_QUERY] Ticket ${ticketId} not found for closing.`
      );
    ...
    if (ticket.status === "closed") {
      ...
      bot.answerCallbackQuery(query.id, {
        text: `Заявка ${ticketId} вже закрита`,
      });
      if (role === "manager") {
    ...
    await ticket.save();
    console.log(
      `[CALLBACK_QUERY] Ticket ${ticketId} closed by ${role || "user"}.`
    );
    ...
    if (role === "manager") {
      try {
        await bot.sendMessage(
          ticket.user_id,
          `🔒 Менеджер завершив листування. Ваше звернення закрито.`,
          { reply_markup: mainMenuKeyboard() }
        );
      } catch (sendError) {
        console.error(
          `[CALLBACK_QUERY] Failed to send 'ticket closed' msg to user ${ticket.user_id}:`,
          sendError.message
        );
      }
      await bot.editMessageText(`Листування по заявці ${ticketId} завершено.`, {
        chat_id: chatId,
        message_id: messageId,
        reply_markup: null,
      });
      ...
      await bot.sendMessage(
        process.env.MANAGER_CHAT_ID,
        `Клієнт ${userName} закрив заявку ${ticketId}.`
      );
    }
    return;
  }
  ...
  if (data === "back_to_ticket_list" && isManager) {
    console.log(
      `[CALLBACK_QUERY] Manager going back to ticket list (defaulting to 'all').`
    );
    const type = "all";
    bot.answerCallbackQuery(query.id);
  ...
  if (data.startsWith("stage_status_")) {
    console.log(
      `[CALLBACK_QUERY] Client asking stage status question: ${data}`
    );
  ...
  if (data === "stage_back") {
    console.log(`[CALLBACK_QUERY] Client going back to stages keyboard.`);
    if (isManager) return bot.answerCallbackQuery(query.id);
    await bot.editMessageText("Виберіть стадію замовлення:", {
      chat_id: chatId,
      message_id: messageId,
  ...
  if (data.startsWith("status_question_")) {
    console.log(
      `[CALLBACK_QUERY] Client asking order status question: ${data}`
    );
    if (isManager) return bot.answerCallbackQuery(query.id);
    let responseText = "";
    switch (data) {
  ...
  if (data === "status_back") {
    console.log(`[CALLBACK_QUERY] Client going back to order status view.`);
    if (isManager) return bot.answerCallbackQuery(query.id);
    const user = await User.findOne({ user_id: chatId });
    if (!user || !user.username) {
      await bot.editMessageText(
        "Не вдалося отримати статус. Перевірте юзернейм.",
    ...
    if (!orders || orders.length === 0) {
      await bot.editMessageText("Замовлень ще немає.", {
        chat_id: chatId,
        message_id: messageId,
        reply_markup: null,
      });
    ...
    switch (status) {
      case "Замовлення прийнято та на етапі купівлі ✅":
        inlineKeyboard = [
          [{ text: "Коли новий статус?", callback_data: "status_question_1" }],
        ];
        break;
      case "Товар викуплено та відправлено на склад в Китаї ✅":
        inlineKeyboard = [
          [{ text: "Коли прибуде?", callback_data: "status_question_2" }],
        ];
        break;
      case "Товар прибув на склад та готується до перевірки ✅":
        inlineKeyboard = [
          [{ text: "Коли фото-звіт?", callback_data: "status_question_3" }],
        ];
        break;
      case "Фотозвіт готовий.":
        inlineKeyboard = [
          [
            {
              text: "Все підходить ✅",
              callback_data: `confirm_photos_${latestOrder.orderId}`,
            },
          ],
        ];
      ...
      case "Посилка успішно скомплектована та готується до відправки ✅":
        responseText += `\nДоставка: ${
          latestOrder.deliveryPrice != null
            ? latestOrder.deliveryPrice + " грн"
            : "Очікуйте"
        } грн.`;
        inlineKeyboard = [
          [{ text: "Час на оплату?", callback_data: "status_question_5" }],
        ];
        break;
      case "Посилка прибула до України та готується до відправлення ✅":
        inlineKeyboard = [
          [{ text: "Коли відправлення?", callback_data: "status_question_6" }],
        ];
        break;
    }
    await bot.editMessageText(responseText, {
  ...
  console.log(
    `[CALLBACK_QUERY] Unhandled callback data: ${data} by User ID: ${userId}`
  );
  bot
